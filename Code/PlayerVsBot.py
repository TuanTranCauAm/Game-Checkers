...
class COMPUTER (tk.Frame):
    ...
    def gameStartupDialog(self):
        ...
        tk.Label(self.start, text="Player: ", font=("Helvetica", 15), height=5, width=20).grid(
            row=1, column=0)
    ...
    def beginGameHelper(self): #Dưới đây là giao diện các phím khi chơi
        ...
        self.offerDrawButton = tk.Button(
            self.game, text="Draw", command=self.offerDraw)
        self.offerDrawButton.grid(row=4, column=1)

        self.acceptDrawButton = tk.Button(
            self.game, text="Are you sure ?", command=self.acceptDraw)
